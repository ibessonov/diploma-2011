...
/*
 *  Класс MathArcsin реализует функцию arcsin(x), переменная и результат вычисления -
 *  вещественные числа.
 *  @author Полевая Евгения
 */

public class MathArcsin extends ExternalFunction {
    ...
    protected final Program initProgram() {
        ...
        temp.append(new Command() {
            ...
            public void execute(ProgramsStack stack, Counter counter)
                ...
                stack.push(new RealElement(Math.asin(x)));
                counter.inc();
            }
        });
        return temp;
