...
/*
 *  Класс MathArctg реализует функцию arctg(x), переменная и результат вычисления -
 *  вещественные числа.
 *  @author Полевая Евгения
 */

public class MathArctg extends ExternalFunction {
    ...
    protected final Program initProgram() {
        ...
        temp.append(new Command() {
            ...
            public void execute(ProgramsStack stack, Counter counter)
                ...
                stack.push(new RealElement(Math.atan(x)));
                counter.inc();
            }
        });
        return temp;
