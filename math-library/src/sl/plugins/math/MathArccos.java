...
/*
 *  Класс MathArccos реализует функцию arccos(x), переменная и результат вычисления -
 *  вещественные числа.
 *  @author Полевая Евгения
 */

public class MathArccos extends ExternalFunction {
    ...
    protected final Program initProgram() {
        ...
        temp.append(new Command() {
            ...
            public void execute(ProgramsStack stack, Counter counter)
                ...
                stack.push(new RealElement(Math.acos(x)));
                counter.inc();
            }
        });
        return temp;
