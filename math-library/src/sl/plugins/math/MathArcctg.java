...
/*
 *  Класс MathArcctg реализует функцию arcctg(x), переменная и результат вычисления -
 *  вещественные числа.
 *  @author Полевая Евгения
 */

public class MathArcctg extends ExternalFunction {
    ...
    protected final Program initProgram() {
        ...
        temp.append(new Command() {
            ...
            public void execute(ProgramsStack stack, Counter counter)
                ...
                stack.push(new RealElement(Math.PI/2-Math.atan(x)));
                counter.inc();
            }
        });
        return temp;
