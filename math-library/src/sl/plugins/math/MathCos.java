...
/*
 *  Класс MathCos реализует функцию cos(x), переменная и результат вычисления -
 *  вещественные числа.
 *  @author Полевая Евгения
 */

public class MathCos extends ExternalFunction {
    ...
    protected final Program initProgram() {
        ...
        temp.append(new Command() {
            ...
            public void execute(ProgramsStack stack, Counter counter)
                ...
                stack.push(new RealElement(Math.cos(x)));
                counter.inc();
            }
        });
        return temp;
